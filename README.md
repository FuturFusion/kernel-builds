...
Both are judged against `misc/<series>/reference-<arch>-config` — `<series>`
comes from `KERNEL_TREE_PATH`'s own version, `<arch>` from the `genconfig.sh`
...
on, `--validate` or not — a real build (e.g. a `.deb`) needs it either way:
...
- **`reference-diff`** — fidelity, one leg per `(series, arch)` that has a
...
- **`all-flavors`** — coverage, one leg per **(flavor, arch)**, all against the
  ...
  automation) and every arch. No reference diff — just that generation
  succeeds.
...
- **Next series** — a version in a series *newer* than the tracked one. Draft
  ...
  auto-merged: a human reviews the (large, cross-series) diff, merges it, and
...
`configs/<version>/<flavor>-<arch>-config` for every flavor × every arch in
`arches`.
...
- **Same series** — there must already be a `configs/<same-series>.*` to diff
  against. A new series (`7.1.x` → `7.2.0`) is always left for a human.
...
the PR for a human, who merges it and pushes the tag themselves.

`.github/workflows/release-on-tag.yml` is the human entry point: pushing any
